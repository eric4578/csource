...
RETURN:无
************************************************************************/
void Input_Vis(char* ip,int x,int y,int lim,int color,int wordcolor) //输入内容，输入框左上角坐标x,y，字数限制，输入框颜色
{                                                      //注意：该输入函数要求输入框宽度为25
	int i=0;
	int j=0;                                           //记录输入数
	char c;                                            //读取键盘输入的内容
	...
	line(x+6,y+4,x+6,y+36);                            //画输入提示光标（在给定坐标向右10个像素点，画一条长14个像素点的竖线
	while(bioskey(1))
	{
		i=bioskey(0);
	}
		...
		if(j<lim)
		{
			if(c!='\n'&&c!='\r'&&c!=' '&&c!=033)       //如果读入非换行、回车、空格或退出
			{
				if(c!='\b')                            //如果读入非退格
				{
					...
					*(ip+j+1)='\0';                    //结束字符串ip
					bar(x+6+j*18,y+4,x+6+j*18,y+36);  //清除光标（注意在原光标坐标上加上已经输入的内容）
					outtextxy(x+8+j*18,y+5,ip+j);
                     //在输入框中显示输入的字符（在其实光标右侧j*18处画字符）
					j++;                               //输入数加一
					line(x+6+j*18,y+4,x+6+j*18,y+36);//在新输入的字符后重画光标
				}
				else if (c=='\b'&&j>0)                 //如果读入退格
				{
					bar(x+6+j*18-18,y+4,x+6+j*18,y+36);  //清除上一输入字符
					bar(x+6+j*18,y+4,x+6+j*18,y+36);  //清除光标
					j--;                               //输入数减一
					line(x+6+j*18,y+4,x+6+j*18,y+36);//在上一输入的字符后重画光标
